...
VERSION="BINANCE_PERP_MOMENTUM_V1"
...
S=requests.Session()
S.headers["User-Agent"]="BinancePerpMomentumV1/1.0"
...
def signal(symbol,m,h,oi):
    if len(m)<BREAKOUT+ATR_N+5 or len(h)<HTF_SLOW+5 or len(oi)<OI_LOOKBACK: return None
    c=m[-2]; prev=m[-3]; a=atr(m)
    ...
    oi_change=(oi[-1]-oi[0])/oi[0] if oi[0]>0 else 0
    long_ok=c["c"]>up and prev["c"]<=up and ef>es and hf>hs and vr>=MIN_VOL_RATIO and move>=MIN_MOVE_PCT and oi_change>=MIN_OI_CHANGE
    short_ok=c["c"]<dn and prev["c"]>=dn and ef<es and hf<hs and vr>=MIN_VOL_RATIO and move<=-MIN_MOVE_PCT and oi_change>=MIN_OI_CHANGE
    if not(long_ok or short_ok): return None
    side="LONG" if long_ok else "SHORT"; entry=c["c"]; sl=entry-STOP_ATR*a if side=="LONG" else entry+STOP_ATR*a
    return {"symbol":symbol,"side":side,"entry":entry,"atr":a,"sl":sl,"breakout":up if side=="LONG" else dn,"vol_ratio":vr,"move_pct":move,"oi_change":oi_change}
...
def main():
    ...
    opened=0; print(f"=== {VERSION} {len(ranked)} symbols ===")
    for item in ranked:
        sym=item["symbol"]
        try:
            ...
            if len(open_trades(s))<MAX_OPEN and not any(t["symbol"]==sym for t in open_trades(s)):
                ...
                if q:
                    ...
                    opened+=1; print(f"OPEN {sym} {q['side']} entry={ep:.8g} vol={q['vol_ratio']:.2f} move={q['move_pct']:.2f}% OI={q['oi_change']*100:.2f}%")
    ...
    print(f"NEW={opened} OPEN={len(open_trades(s))} CLOSED={len(closed)} BAL={s['balance']:.2f} WR={wr:.1f}% NET={sum(wins)+sum(losses):+.2f} PF={pf:.2f}")
